...
router.get('/users', async (req, res, next) => {
  try {
    ...
    const { rows: users, count } = await userService.getAllUsers({ search, limit, offset });
...
router.post('/users/:id/edit', async (req, res) => {
  try {
    ...
    if (existingRoleName === 'admin' && parseInt(role_id, 10) !== existingUser.role_id) {
...
router.get('/roles', async (req, res, next) => {
  try {
    ...
    const limit = 50;
    const offset = (page - 1) * limit;
    const { rows: roles, count } = await roleService.getAllRoles({ search, limit, offset });
    const totalPages = Math.ceil(count / limit);
    res.render('admin/roles/index', { title: 'Manage Roles', roles, currentPage: page, totalPages, search });
  } catch (err) {
    next(err);
  }
});
...
router.get('/flights', async (req, res, next) => {
  try {
    ...
    if (search) {
      flightsFiltered = flightsFiltered.filter(f =>
    ...
    const totalPages = Math.ceil(count / limit);
    res.render('admin/flights/index', { title: 'Manage Flights', flights, currentPage: page, totalPages, search });
  } catch (err) {
    next(err);
  }
});
...
router.post('/flights/new', async (req, res, next) => {
  try {
    ...
    res.render('admin/flights/new', { title: 'Create Flight', error: err.message });
  }
});
...
router.get('/flights/:id/edit', async (req, res, next) => {
  try {
    ...
    res.render('admin/flights/edit', { title: 'Edit Flight', flight, error: null });
  } catch (err) {
    next(err);
  }
});
...
router.get('/airport-slots', async (req, res, next) => {
  try {
    ...
    const totalPages = Math.ceil(count / limit);
    res.render('admin/airportSlots/index', { title: 'Manage Airport Slots', slots, currentPage: page, totalPages, search });
  } catch (err) {
    next(err);
  }
});
...
router.get('/airport-slots/new', (req, res) => {
  res.render('admin/airportSlots/new', { title: 'Create Airport Slot', error: null });
});

router.post('/airport-slots/new', async (req, res, next) => {
  try {
    const { airport_code, date, time_slot_start, time_slot_end, slot_capacity } = req.body;
    ...
    res.render('admin/airportSlots/new', { title: 'Create Airport Slot', error: err.message });
  }
});
...
router.get('/airport-slots/:id/edit', async (req, res, next) => {
  try {
    ...
    res.render('admin/airportSlots/edit', { title: 'Edit Airport Slot', slot, error: null });
  } catch (err) {
    next(err);
  }
});
...
router.post('/airport-slots/:id/delete', async (req, res, next) => {
  try {
    ...
    if (!success) return res.status(404).send('Airport Slot could not be deleted.');
    res.redirect('/admin/airport-slots');
  } catch (err) {
    next(err);
  }
