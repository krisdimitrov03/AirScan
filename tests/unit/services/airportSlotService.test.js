...
describe("airportSlotService", () => {
  ...
  describe("updateSlot", () => {
    it("updates the slot when found", async () => {
      ...
      const mockSlotInstance = {
        update: jest.fn().mockResolvedValue({ ...slotData, slot_id: slotId }),
      };
    ...
    it("returns null if slot not found", async () => {
      ...
      const result = await airportSlotService.updateSlot(999, {
        slot_capacity: 150,
      });
  ...
  describe("deleteSlot", () => {
    it("returns true when deletion is successful", async () => {
      ...
      expect(AirportSlot.destroy).toHaveBeenCalledWith({
        where: { slot_id: 1 },
      });
